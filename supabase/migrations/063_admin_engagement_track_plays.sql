-- Panel admin: incluir reproducciones de pistas (charts / previews / YouTube inline)
-- además de las de mixes (mix_play_events).
...
AS $$
  SELECT jsonb_build_object(
    ...
    'track_plays_summary', (
      SELECT jsonb_build_object(
        'all_time', (SELECT COUNT(*)::bigint FROM public.track_play_events),
        'last_7d', (
          SELECT COUNT(*)::bigint FROM public.track_play_events
          WHERE created_at >= NOW() - INTERVAL '7 days'
        )
      )
    ),
    'track_plays_top', (
      ...
      FROM (
        SELECT
          e.canonical_key,
          CASE
            WHEN e.canonical_key LIKE 'yt:%' THEN 'YouTube · ' || substr(e.canonical_key, 4)
            WHEN e.canonical_key ~ '/track/[^/]+/' THEN initcap(
              replace(
                regexp_replace(e.canonical_key, '^.*/track/([^/]+)/.*$', '\1'),
                '-',
                ' '
              )
            )
            ELSE e.canonical_key
          END AS title,
