-- ============================================
-- OPTIMAL BREAKS — Reproducciones de mixes (in-app) + RPC stats admin
-- No tracking de previews Beatport / chart.
-- ============================================

CREATE TABLE IF NOT EXISTS public.mix_play_events (
...
CREATE INDEX IF NOT EXISTS idx_mix_play_events_mix ON public.mix_play_events(mix_id);
CREATE INDEX IF NOT EXISTS idx_mix_play_events_created ON public.mix_play_events(created_at DESC);

ALTER TABLE public.mix_play_events ENABLE ROW LEVEL SECURITY;

-- Sin políticas: anon/authenticated no leen/escriben; service_role bypassa RLS.

-- ============================================
-- Panel admin: un JSON con todos los rankings (solo service_role vía API)
-- ============================================
