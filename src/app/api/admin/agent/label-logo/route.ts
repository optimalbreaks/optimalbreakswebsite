...
export async function POST(request: NextRequest) {
  ...
  const query = `"${labelName}" record label logo discography music`.replace(/\s+/g, ' ').trim()
  const alternateQueries = [
    `"${labelName}" electronic music label logo`,
    `${labelName} records label logo`,
    `"${labelName}" discogs label`,
    `${labelName} breakbeat label logo`,
  ]
