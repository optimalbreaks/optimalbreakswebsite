...
**TIDAL** funciona igual con `--service=tidal` (`npm run db:chart:tidal`, columna **`tidal_url`**, migración **`067_charts_tidal_url.sql`**, env `TIDAL_CLIENT_ID` + `TIDAL_CLIENT_SECRET` de developer.tidal.com — sin requisito Premium ni cuota diaria conocida). Diferencia: el **botón TIDAL solo sale con enlace verificado** (sin fallback de búsqueda), porque su catálogo de breaks es más limitado. Los botones viven en `TrackShareButton.tsx` (`SpotifyLinkButton`, `TidalLinkButton`, `BeatportLinkButton`): **circulares con logo de marca en móvil**, pastillas de texto en escritorio; se usan en `/charts`, Top 10 de artista/sello, New Releases del artista y Mis Tracks (propia y lista pública).

Los syncs semanales **no** pisan los matches: la RPC del 40 (`apply_chart_tracks_row_updates`) no incluye las columnas y `chart-featured-upsert.mjs` solo envía `spotify_url` / `tidal_url` si vienen en el JSON. Tras publicar edición nueva: `npm run db:chart:spotify -- --week=<lunes>` (y `db:chart:tidal`). El OAuth por usuario (añadir a playlist, reproducción completa embebida) queda **descartado**: desde feb/mar-2026 las apps en Development Mode admiten máx. 5 usuarios en allowlist y el Extended Quota Mode exige organizaciones con ≥250k usuarios activos mensuales.
