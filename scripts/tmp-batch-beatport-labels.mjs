/**
 * Temporary script: search Beatport for labels that don't have beatport_id yet.
 * Fetches Beatport search, extracts label ID, then scrapes Top 10 tracks.
 * Usage:
 *   node scripts/tmp-batch-beatport-labels.mjs --dry-run --limit=5
 *   node scripts/tmp-batch-beatport-labels.mjs
 */
...
const { data: labels, error } = await sb.from('labels').select('slug, name, is_active, beatport_id').order('slug')
if (error) { console.error(error); process.exit(1) }

const candidates = labels.filter(l => !l.beatport_id && l.is_active !== false)
console.log(`\n  Total sellos: ${labels.length}`)
console.log(`  Ya con beatport_id: ${labels.filter(l => l.beatport_id).length}`)
console.log(`  Candidatos (activos sin beatport_id): ${candidates.length}\n`)
...
async function scrapeAndUpsertLabel(slug, bpSlug, bpId) {
  ...
  try {
    ...
    if (!dryRun) {
      const { error: upErr } = await sb.from('labels').update({
        beatport_url: bpUrl,
        beatport_id: bpId,
...
for (const l of candidates.slice(0, maxCount)) {
  processed++
  process.stdout.write(`  [${processed}/${Math.min(maxCount, candidates.length)}] ${l.name} (${l.slug})...`)

  const bp = await searchBeatportLabel(l.name)
  if (!bp) {
  ...
  const count = await scrapeAndUpsertLabel(l.slug, bp.slug, bp.id)
  if (count > 0) {
    ...
    if (!dryRun) {
      await sb.from('labels').update({ beatport_id: bp.id, beatport_url: `https://www.beatport.com/label/${bp.slug}/${bp.id}` }).eq('slug', l.slug)
    }
    found++
  }
