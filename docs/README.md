...
| Ubicación | Propósito |
|-----------|-----------|
| **[`README.md`](../README.md)** (raíz) | Documentación técnica **canónica** (inglés): stack, env, auth, Storage, DJ deck, **sistema de audio global** (`DeckAudioProvider`, `claimAudio`, cinco fuentes mutuamente exclusivas, barra flotante compartida), **línea temporal de la home** (`home.section_history` en diccionarios: orden **editorial manual**, sin sort por año), **tira de eventos** (hasta 4 próximos / fallback), migraciones (tabla parcial), scripts `npm`, estructura de carpetas, **listado de eventos** (`EventsExplorer`: semáforo, hover, `group/link`), **CTA MonsterTicket** en ficha de evento, **página mixes** (filtros, embeds perezosos YouTube/SoundCloud, `MixesExplorer`), **Beatport Top 10** en fichas (`046`, `db:beatport:top`, vs chart `db:chart`). |
...
- **Listado `/events` (`EventsExplorer`):** pie **semáforo** (pasado `var(--red)` + texto blanco; próximo `var(--yellow)` + `var(--ink)`), hover con `color-mix`, franja detrás del cartel alineada al estado, tarjeta **`group/link`**, zoom del cartel vía **`CardThumbnail`** con **`groupHoverGroup="link"`**, pies con altura homogénea en la rejilla. Detalle en **`README.md`** (*Directory listing views*).
