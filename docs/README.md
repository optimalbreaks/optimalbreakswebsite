...
| Ubicación | Propósito |
|-----------|-----------|
| **[`README.md`](../README.md)** (raíz) | Documentación técnica **canónica** (inglés): stack, env, auth, Storage, DJ deck, **sistema de audio global** (`DeckAudioProvider`, `claimAudio`, cinco fuentes mutuamente exclusivas, barra flotante compartida), **línea temporal de la home** (`home.section_history` en diccionarios: orden **editorial manual**, sin sort por año), migraciones (tabla parcial), scripts `npm`, estructura de carpetas, **página mixes** (filtros, embeds perezosos YouTube/SoundCloud, `MixesExplorer`), **Beatport Top 10** en fichas (`046`, `db:beatport:top`, vs chart `db:chart`). |
| **[`README.es.md`](../README.es.md)** | Resumen en **español**: flujo de artistas, auth, GA4, vistas de listado (incl. **mixes**: filtros y embeds), **sistema de audio global** (exclusión mutua, barra flotante), **Beatport Top 10** en artistas/sellos (`db:beatport:top`), **misma nota sobre la timeline de la home**; enlaza al README en inglés para el detalle largo. |
