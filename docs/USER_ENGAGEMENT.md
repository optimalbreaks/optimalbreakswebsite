...
### Track-level deep-linking (share → tap-to-play on Optimal Breaks)

Problem this solves: sharing a song via Beatport / Bandcamp links sends the receiver **off** the site. The goal is that every shared song opens inside Optimal Breaks with the **"TAP TO PLAY" modal** armed on the exact track — one tap and it plays (product rule, Sep 2026: shared links never autoplay; see below).

Single URL scheme per source:

| Source | Shared URL | Destination component |
|--------|-----------|------------------------|
| `chart` (40 Breaks Vitales) | `/[lang]/charts?week=<YYYY-MM-DD>&play=chart:<uuid>` | `ChartView` opens the matching weekly edition, scrolls + highlights the row and arms the **`TapToPlayOverlay`** modal; the receiver's tap calls `playPreviewQueue` (via **`usePreviewAudioGated`** → loads engine if needed) |
| `featured` (New Releases) | `/[lang]/charts?week=<YYYY-MM-DD>&play=featured:<uuid>` | same as above |
| `beatport_top` (artist / label Top 10) | `/[lang]/<artists|labels>/<slug>?play=beatport:<beatportId>` | `BeatportTopTracks` expands the Top 10 accordion on that profile, scrolls to the row and arms the same **`TapToPlayOverlay`** modal (its `pendingTapPlay` state); the tap calls `playPreviewQueue` |
...
The **admin users drawer** (`/[lang]/administrator/users` → click a Tracks count) uses this **same** URL scheme. A `beatport_top` save must open the artist/label ficha with `?play=beatport:<id>` (expand Top 10, scroll, highlight, tap-to-play modal) — **never** `canonical_url` / Beatport. Helpers: `buildBeatportTopInternalPath`, `resolveBeatportPlayId`, `beatportShareOriginFromSnapshot` in `src/lib/share-track.ts`. Origin comes from `snapshot.origin` (`kind` + `slug`); if an old save has `origin.id` but no slug, `GET /api/admin/users/[id]/engagement` resolves it. If the track has dropped out of that ficha’s current Top 10, the page still opens and the track is **rescued** so the link never goes mute (see below).
...
Net effect: when you paste the URL into WhatsApp / X / Signal, the preview shows the **track artwork and name** (not a generic chart / profile card), and clicking it lands on Optimal Breaks with the tap-to-play modal armed on that song — one tap plays it.

**Shared links: modal, never autoplay (product rule, Sep 2026).** Shared `?play=<source>:<id>` links do **not** attempt `audio.play()` on landing. `ChartView` (with `pendingPlay.autoplay = false`) and `BeatportTopTracks` (with `pendingTapPlay`) arm the shared **`TapToPlayOverlay`** (`src/components/TapToPlayOverlay.tsx`) — cover art + title + artist + red ▶ — and the receiver's tap starts playback (a real gesture, so identical on desktop and mobile). The old pattern ("attempt autoplay, show a modal when the browser rejects with `NotAllowedError`") never showed the modal on mobile: WebKit rejects the `pause()→src→play()` sequence with `AbortError` instead. Do **not** change the share URL formats — links already published on Instagram must keep working.

**Shared links never go mute (rescue fallback, Sep 2026).** Beatport Top 10 lists rotate on every re-scrape, so an old `?play=beatport:<id>` link can point at a track no longer in the ficha's current JSONB. Product rule: **a link to a song that was on the site must keep playing.** Server side, `findBeatportTopFallbackTrack` (`src/lib/beatport-top-fallback.ts`) recovers the track from, in order: `chart_tracks` (40 Breaks), `chart_featured_tracks` (New Releases) — both via the cached public client — and finally **any user's `beatport_top` save snapshot** in `saved_chart_tracks` (service role; snapshots froze title/artists/artwork/sample at save time). The artist/label pages pass the recovered track to `<BeatportTopTracks fallbackTrack>`, which arms the same `TapToPlayOverlay` with a single-track queue (no extra row rendered, no scroll); the OG override in `generateMetadata` uses the same helper so WhatsApp previews keep showing the song. Only if all three sources miss does the ficha open without a modal.

**Autoplay fallback (⌘K flow only).** The global search deep-link (`#chart-row-<id>` + `?play=1`, an in-session gesture) still auto-starts. If the browser blocks it with `NotAllowedError`, `DeckAudioProvider` flips `previewBlocked: true` (exposed on **`DeckAudioContext`** / **`usePreviewAudio().previewBlocked`**) and renders its full-screen **`PreviewAutoplayOverlay`** — same "▶ TAP TO PLAY" card. One tap calls `togglePreview()` inside a gesture and the overlay self-dismisses (`previewBlocked` clears on successful play and on `stopPreview`). Any other play error (bad URL, CORS, etc.) does **not** trigger the overlay.
...
- `src/components/BeatportTopTracks.tsx` — resolves `?play=beatport:<id>` inside the artist/label profile (expand accordion + scroll + `TapToPlayOverlay` modal; the tap plays) and embeds a `TrackShareButton` per row.
- `src/components/TapToPlayOverlay.tsx` — shared "TAP TO PLAY" modal for shared track links (used by `ChartView` and `BeatportTopTracks`); the receiver's tap is the gesture that starts playback.
