...
**Community Top** (`/[lang]/top100`; `/charts` only links there): `CommunityMonthlyTop.tsx` calls **`GET /api/public/charts/community-monthly`** (`limit` up to 100; the page requests 100). All-time ranking of **"+" saves** in My Tracks — same pass for **top 100 tracks** and the **artist board** (API top **50**; heading stays «Top 10»; first 10 rows, **Load more** / **Show less**). Artist rows use the same movement language as 40 Breaks Vitales (**NUEVO** / **▲** / **▼** / **═**): the **list is live** (ranks move with each save); the **variation compares now vs the previous ISO Monday** (previous Monday 00:00 UTC snapshot, reconstructed from `created_at`, no snapshot table — so a weekend overtake keeps its ▲/▼ through the following week). `X sem.` = consecutive weeks **on the board**, not at that rank; the #1 only also gets **weeks at #1** plus a one-line editorial headline. Canonical dedupe as the admin Tracks dashboard; tracks sorted by **unique users → total saves → play count → most-recent save → alphabetical**. Source-row lookups **must** use chunked `.in()` (`IN_CHUNK = 200` in `community-monthly/route.ts`, same idea as `/api/public/user-tracks`): one giant `.in('id', …)` on New Releases drops metadata and silently drops saves that have no `snapshot`. The slug `community-monthly` is preserved for compatibility (was monthly until April 2026). A remix save also credits the **remixer** (`mix_name` / Beatport `remixers[]`, `src/lib/remixer-credits.ts`); the **track** Top 100 stays one row per song. **Self-credits:** an **editorially marked** account (`editorial_artist_marks`, admin Users) or an **approved claim** (`claimed_by`) does not add credits to *their own* name on the artist board (the board stays ranked by **songs saved**). **Label mark** (`editorial_label_marks`): that account’s saves of that label credit **nobody** on the artist board (My Tracks + track Top 100 unchanged); collabs on **other** labels still count. Bookings stay on claim + `accepts_bookings`. After a mark, leftover `N saves · M fans` are **other** users (often the editorial login), not a broken skip — Afghan Headspin, 25 Aug 2026 (24 self-saves skipped); DKR dump 26 Aug; **who still lifts them after both marks:** J-Break 26 = Optimal Breaks 19 + Afghan 5 (non-DKR) + jennie 1 + Mestas 1; Afghan Headspin 21 = Optimal Breaks 17 + Mestas 3 + jennie 1. Marks + emails: [`docs/USER_ENGAGEMENT.md`](docs/USER_ENGAGEMENT.md) (*Editorial marks in production*). Migrations **`070`** / **`071`**. Detail: [`docs/USER_ENGAGEMENT.md`](docs/USER_ENGAGEMENT.md) (*Community Top* → *Artist board — weekly movement*).

**Soulmates** (`/[lang]/mi-cuenta/almas-gemelas`): **`GET /api/breakbeat/soulmates`** (authenticated cookie session) computes **Jaccard similarity** over canonical keys vs other users who opted in; returns top matches plus cross-user recommendations. A **marked/claimed** account does **not** put self-credited tracks in *their* Jaccard set (same identity as the artist board); My Tracks and the track Top 100 still count those «+». Toggle **public list for Soulmates / Community Top** on `/mi-cuenta/perfil`.
...
**Per-track sharing.** Every row on chart, saved-tracks and Beatport Top 10 lists carries a per-song `TrackShareButton` (🔗) that copies a URL of the form `/charts?week=…&play=<chart|featured>:<uuid>` or `/<artists|labels>/<slug>?play=beatport:<beatportId>`. Opening that URL lands on Optimal Breaks with the **"TAP TO PLAY" modal** armed on that song (one tap plays it — shared links never autoplay, product rule Sep 2026) and the preview card (artwork + title) rendered on social media via server-side OG overrides. Details in *[Per-track sharing](#per-track-sharing-open--tap-to-play-on-optimal-breaks)* and `docs/USER_ENGAGEMENT.md`.
...
**Admin Users.** `/[lang]/administrator/users` lists Auth + profile accounts. Clicking Favorites / Mixes / Tracks counts opens `AdminUserEngagementDrawer`. Track rows use the same internal `?play=` URLs as share/player: a **Beatport Top 10** save goes to `/[lang]/artists|labels/<slug>?play=beatport:<id>` (never Beatport.com). Search matches **email, display name and username**; clearing the field reloads the full list immediately (debounce + ignore stale responses). Spec: [`docs/USER_ENGAGEMENT.md`](docs/USER_ENGAGEMENT.md) (*Admin users*).
...
**Label page order** (Aug 2026 redesign): header (logo, title, favorite, **Beatport Top 10** accordion, **“On Optimal Breaks”** accordion — the label’s 40 Breaks + New Releases + archive (YouTube/Bandcamp) picks via `ArtistFeaturedTracks` with `origin.kind: 'label'`, fed by `fetchLabelOnSitePicks` in `src/lib/artist-related-content.ts` — and the Discogs CTA) → **roster carousel** → bio + sidebar (key releases + links only).
...
- **On Optimal Breaks accordion** (`ArtistFeaturedTracks`, `fetchArtistFeaturedPicks`): lists every editorial pick — 40 Breaks, New Releases, archive Beatport/Bandcamp, and archive YouTube (`chart_vinyl_tracks`) — where the artist is in **`artists[]`** *or* is the **remixer** (`mix_name` such as `Jem Haynes Remix`, plus Beatport `remixers[]` on import). Same helper as charts / Top 100: **`src/lib/remixer-credits.ts`**. A remixer-only credit used to live only in `mix_name`, so the accordion stayed empty (e.g. Jem Haynes / *I WANNA*). Generic labels (`Original Mix`, `VIP Remix`, `Breakbeat Remix`) do **not** invent an artist. The accordion is a **unique song list**: the same title + artists counts once even if it exists as a single and as an album cut (different Beatport IDs) or as Original Mix vs Instrumental. Catalog rows are **not** deleted (saves keep their UUIDs); the merge stores every copy in `relatedRefs` so the "+" still lights up. Named remixes stay distinct. Helper: `trackDisplayIdentityKey` in `src/lib/track-canonical-key.ts`.
...
| Community Top 100 | `/[lang]/top100` | All-time ranking of **"+" saves** (My Tracks): artist board (10 visible, load more to 50, weekly ▲/▼/NUEVO; remixer names count) + top 100 tracks. API `GET /api/public/charts/community-monthly` (chunked `.in()` so every hydratable save counts) |
...
| Admin users | `/[lang]/administrator/users` | Auth + profiles. Counts open an engagement drawer (internal `?play=` links; Beatport Top 10 → ficha). Search: email + name + username. Editorial marks on `/users/[id]`. |
...
`parsePlayParam` in `src/lib/share-track.ts` normalises the three shapes plus the legacy `?play=1` (and `vinyl:<uuid>` for in-app deep links). Retro Vinyl Picks are **not** shared to the public this way — they stay on their Discogs / YouTube links because iframe-only playback makes autoplay via arbitrary share links unreliable. The **admin users drawer** still uses internal `?play=` for live vinyl rows and **must** send `beatport_top` saves to `/artists|/labels/<slug>?play=beatport:<id>` (`buildBeatportTopInternalPath`), never to Beatport.com.
...
**Shared links: modal, NEVER autoplay (product rule, Sep 2026).** A shared link (`?play=chart:|featured:<uuid>` on /charts, `?play=beatport:<id>` on artist/label profiles) does **not** call `audio.play()` on landing. Instead the destination arms the shared **`TapToPlayOverlay`** (`src/components/TapToPlayOverlay.tsx`: cover art + title + artist + red ▶) and the **receiver's tap** starts playback — a real user gesture, so it behaves identically on desktop and mobile. `ChartView` arms it via `pendingPlay.autoplay = false`; `BeatportTopTracks` via its `pendingTapPlay` state. Rationale: the old pattern ("attempt autoplay, show the modal only when the browser rejects with `NotAllowedError`") **never showed the modal on mobile**, because WebKit rejects the `pause()→src→play()` sequence with `AbortError`, not `NotAllowedError` (Sep 2026 bug). The only flow that still auto-starts is the global search (⌘K → `#chart-row-<id>` + `?play=1`, an in-session gesture); if that attempt is blocked with `NotAllowedError`, `DeckAudioProvider` flips `previewBlocked` and renders its **`PreviewAutoplayOverlay`** (same look) whose tap calls `togglePreview()`. Do **not** change the share URL formats — links already published on Instagram must keep working.

**Shared links never go mute (rescue fallback).** Beatport Top 10s rotate on every re-scrape, so an old `?play=beatport:<id>` can point at a track no longer in the ficha's current JSONB. **`findBeatportTopFallbackTrack`** (`src/lib/beatport-top-fallback.ts`, server-only) recovers it from `chart_tracks` → `chart_featured_tracks` → any user's `beatport_top` save **snapshot** (service role), and the artist/label pages pass it to `<BeatportTopTracks fallbackTrack>`: same tap-to-play modal, single-track queue, no extra row. The OG override uses the same helper. Details in `docs/USER_ENGAGEMENT.md` (*Shared links never go mute*).

**Cover art in the overlay.** Beatport artwork URLs must not be loaded with a raw `<img>`: their CDN often returns **403** when the browser sends a non-Beatport `Referer`. The overlay therefore uses **`next/image`** (`fill`, `sizes`) so the request goes through **`/_next/image`** — same pattern as chart rows and `BeatportTopTracks`. Hostnames must remain allowed in `next.config.js` → `images.remotePatterns`. If the image still fails, `onError` swaps the thumbnail for a neutral **♪** placeholder instead of the broken-image icon. The shared `TapToPlayOverlay` uses a plain `<img>` with an ordered `artworkCandidates` list instead (proxied URL first via `proxyCatalogArtworkForDisplay` → `/api/og/image-proxy`, then the raw artwork, ♪ placeholder last), stepping to the next candidate on error.
...
- **Audio-focus interruptions behave like a real music app (`previewInterruptedRef`).** If the OS pauses us **after we were already hidden for >1 s** (music was playing in the background and another app grabs the speaker — WhatsApp voice note, a call), we mark interrupted **immediately** and **do not** call `play()`. On mobile that `play()` succeeds and steals audio from WhatsApp (the ~2 s cut). The single ~1.5 s retry is **only** for the pause that coincides with going to background / lock (OS throttling); if that `play()` is rejected we also back off. Once interrupted, neither the keeper nor the `visibilitychange` resume revive it — the user presses play (or any explicit play / track change / `ended`→advance clears the flag). Benign background pauses still auto-resume on foreground return.
...
- **Media Session:** the deck effect must **not** clear `mediaSession` metadata/handlers while `mode === 'preview'` (it used to, leaving the iOS lock screen orphaned). Preview owns its session via **`src/lib/now-playing-session.ts`** (`applyNowPlaying` in `loadAndPlayPreviewAt` + its handler effect). That helper fills `title` (plus mix name when useful), `artist`, `album` (label) and **same-origin** artwork through `/api/og/image-proxy` (iOS drops the whole session if Beatport JPEGs arrive without CORS). As a fallback it mirrors now-playing into `document.title`, so a failed Media Session no longer shows “My tracks | Optimal Breaks” on the lock screen. It re-applies on lock (`visibilitychange`) and on in-app navigation. YouTube/SoundCloud push the same metadata when their iframe is mounted.
...
- **Link capture (opening shared links inside the installed PWA) is an OS decision — platform limitation, not ours (Sep 2026).** On **Android**, the WebAPK Chrome mints on install registers the domain, so links tapped in other apps usually open straight in the PWA (user-controllable under Settings → Apps → Optimal Breaks → "Open by default"; Instagram/WhatsApp in-app browsers may still hijack the first tap). On **iOS there is no mechanism whatsoever** (no manifest field, meta tag or URL scheme) to make a link open the home-screen web app: shared links always open in the browser; universal links are native-App-Store-apps only. **Do not burn time trying.**
...
- **`mediaSession`** — `metadata` (title, artist, album, artwork) + `play` / `pause` / `previoustrack` / `nexttrack` handlers for hardware media keys, lock screen and Bluetooth. Shared helper: `src/lib/now-playing-session.ts`.
- **OS “Now Playing” / lock screen** — Standard `MediaMetadata` fields only (`title`, `artist`, `album`, `artwork`) plus position state. **No dedicated release-year field.** We keep those strings **clean of years**; year/date stay in the in-app UI. Artwork is served same-origin (`/api/og/image-proxy`) so iOS will actually paint the cover. While audio is playing we also set `document.title` to `Title (mix) · Artist`: that is what iOS shows if it ignores Media Session (previously the page title, e.g. “My tracks | Optimal Breaks”). Re-applied on lock and on client navigation. YouTube and SoundCloud use the same helper.
...
### `PreviewAutoplayOverlay` (⌘K autoplay fallback)

When `loadAndPlayPreviewAt` hits **`NotAllowedError`**, the provider sets **`previewBlocked`** and renders a full-viewport modal above the page: one tappable card ("▶ TAP TO PLAY" / ES equivalent) that calls **`togglePreview()`** on first user gesture. Cover art uses **`next/image`** so Beatport URLs load via **`/_next/image`** (avoids CDN hotlink 403 from plain `<img>`). Failed loads show a **♪** placeholder. Clears when playback starts successfully or **`stopPreview`** runs. Implemented inside `DeckAudioProvider.tsx`. **Note:** shared links no longer rely on this mechanism — they arm the shared `TapToPlayOverlay` directly without attempting `play()` (product rule, Sep 2026); this provider overlay remains as the fallback for the ⌘K flow.
...
- **blog_posts** — title, content, excerpt (EN/ES), category, tags, author, `image_url`, published flag, `view_count` (lecturas de la ficha; `077_blog_post_view_count.sql`)
...
| `070_editorial_artist_marks.sql` | **Editorial artist mark (phase 2):** table **`editorial_artist_marks`** (`user_id` + normalized credit name). Service-role only. Those saves still count on the track Top 100 / My Tracks; they do **not** credit that artist name on `/top100` artists. An approved claim (`claimed_by`) applies the same skip and unlocks bookings. Admin: `/administrator/users/[id]`. |
| `071_editorial_label_marks.sql` | **Editorial label mark:** table **`editorial_label_marks`** (`user_id` + normalized label name as on tracks). Service-role only. Saves of that label by that account still count on the track Top 100 / My Tracks; they do **not** credit **anyone** on `/top100` artists. Not an ownership claim. Admin: `/administrator/users/[id]` → Marcar sello. |
...
- **Remixer = artist credit (Aug 2026):** Beatport stores the original in `artists[]` and the remixer in `remixers[]` / `mix_name` (`PhoenixRising Remix`, `Jem Haynes Remix`). On this site the remixer is the **producer of the breakbeat version** and must appear as a named credit: import merges `artists` + `remixers` + names parsed from `mix_name` (`src/lib/remixer-credits.ts`, `scripts/lib/remixer-credits.mjs`); `ArtistNames` appends them on every track row; artist pages and the **artist board** (`/top100`) credit the remixer (deduped if already in `artists[]`). Skip generic mix labels (`Original Mix`, `VIP Remix`, `Breakbeat Remix`, year-only `2026 Remix`). Re-apply to live rows + pick JSON: `node scripts/chart-featured-upsert.mjs --backfill-remixer-credits` (UPDATE only, same UUIDs).
...
- **Weekly chart (“40 Breaks Vitales”)** — `npm run db:chart` runs `scripts/chart-40-breaks.mjs`: a **literal snapshot of the first 40 positions** of Beatport’s live Breaks / Breakbeat / UK Bass Top 100, in Beatport order. **No AI since Sep 2026** (editions before that were reordered by an OpenAI prompt; `--ai` re-enables that legacy step on request). ▲/▼ therefore reflects real sales movement; the previous-edition lookup skips weeks that have New Releases but no 40. Optimal’s own curation lives in **New Releases**. Separate from per-artist sales widgets.
...
| `npm run db:chart:featured -- data/charts/picks/<week>.json` | **New Releases** UPSERT (`chart-featured-upsert.mjs` → `chart_featured_tracks`). Required after batch/JSON edits for production. Merges remixers into `artists[]`. Standalone: `node scripts/chart-featured-upsert.mjs --backfill-remixer-credits`. |
