...
**Visual viewport sync (iOS PWA).** Both **`MiniPlayerShell`** (the bottom bar) and **`BackToTop`** subscribe to **`window.visualViewport`** (`resize` / `scroll` / `pageshow`) and add `window.innerHeight − (visualViewport.height + visualViewport.offsetTop)` to their `bottom` offset. In iOS standalone, after lock/unlock or when the system bar height changes, the layout viewport can drift from the visual viewport and `bottom: 0` ends up "floating" mid-screen; this compensation keeps the player and the back-to-top button anchored to the visible bottom edge.
...
- **PWA standalone (iOS) anchoring** — The bar is portaled to **`document.body`** (`#ob-audio-overlays`) so no wrapping element can break `position: fixed`, and its `bottom` is a dynamic value that **compensates for `visualViewport` drift** after lock/unlock. Same compensation lives in **`BackToTop`** so both overlays stay glued to the visible bottom edge in standalone mode.
...
- **Player floating mid-screen in iOS PWA after lock/unlock.** In standalone display mode the layout viewport and the visual viewport can desync (system bar height changes, screen lock, status-bar interactions). Plain `position: fixed; bottom: 0` then renders the bar somewhere in the middle of the page with songs still visible behind it. **Fix:** `MiniPlayerShell` (and `BackToTop`) subscribe to `window.visualViewport`'s `resize` / `scroll` / `pageshow` events and add `innerHeight − (vv.height + vv.offsetTop)` to their `bottom`. Combined with the portal to `<body>`, the bar stays glued to the visible bottom edge in every viewport state (PWA, in-browser, after sleep/wake).
...
| `src/components/BackToTop.tsx` | Listens for `OB_CHART_PLAYALL_BAR_EVENT` to offset the scroll button (matches the new player height: `safe-area + 10 px`). Also subscribes to **`window.visualViewport`** and adds the same drift compensation as the player, so on iOS PWA standalone the button stays anchored to the visible bottom edge after lock/unlock. |
